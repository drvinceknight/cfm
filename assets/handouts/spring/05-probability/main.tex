...
\begin{enumerate}
    \item Write a function \mintinline{python}{sample_experiment} that simulates a given ball
        and returns the bowl type (as a string with value
        \mintinline{python}{"backhand"} or
        \mintinline{python}{"forehand"}) and whether it was caught (as a
        boolean: either \mintinline{python}{True} or \mintinline{python}{False}).
    \item Using 1,000,000 samples create a variable
        \mintinline{python}{probability_of_hit} which
        has value an estimate for the probability of the ball being hit.
    \item Using the above, create a variable
        \mintinline{python}{probability_of_spin_given_hit} which has value an estimate for the
